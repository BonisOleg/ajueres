...
class LegalPagesTests(TestCase):
    ...
    def test_privacy_and_offer_pages(self):
        ...
        self.assertContains(offer, 'р/с:')
        self.assertContains(offer, 'Банк:')
        self.assertContains(offer, 'Адрес:')

    def test_requisites_values_render(self):
        doc = LegalDocument.objects.get(slug='offer')
        doc.requisites = [{'label': 'ИНН', 'value': '123456789'}]
        doc.requisites_ru = doc.requisites
        doc.save()
        offer = self.client.get(reverse('offer'))
        self.assertContains(offer, '123456789')
        self.assertContains(offer, 'ИНН:')
        self.assertContains(offer, 'Банк:')
